...
AABB getAABBForTriangle(const Triangle tri) {
	AABB aabb;
	aabb.min = glm::vec3(
		glm::min(glm::min(tri.v[0].x, tri.v[1].x), tri.v[2].x),
		glm::min(glm::min(tri.v[0].y, tri.v[1].y), tri.v[2].y),
		glm::min(glm::min(tri.v[0].z, tri.v[1].z), tri.v[2].z));
	aabb.max = glm::vec3(
		glm::max(glm::max(tri.v[0].x, tri.v[1].x), tri.v[2].x),
		glm::max(glm::max(tri.v[0].y, tri.v[1].y), tri.v[2].y),
		glm::max(glm::max(tri.v[0].z, tri.v[1].z), tri.v[2].z));
	return aabb;
}
// CHECKITOUT
/**
...
glm::vec3 calculateBarycentricCoordinate(const Triangle tri, glm::vec2 point) {
	float beta = calculateBarycentricCoordinateValue(
		glm::vec2(tri.v[0].x, tri.v[0].y),
		point,
		glm::vec2(tri.v[2].x, tri.v[2].y),
		tri.v
		);
	float gamma = calculateBarycentricCoordinateValue(
		glm::vec2(tri.v[0].x, tri.v[0].y),
		glm::vec2(tri.v[1].x, tri.v[1].y),
		point,
		tri.v
		);
...
float getZAtCoordinate(const glm::vec3 barycentricCoord, const Triangle tri) {
	return -(barycentricCoord.x * tri.v[0].z
		+ barycentricCoord.y * tri.v[1].z
		+ barycentricCoord.z * tri.v[2].z);
}
